...
#include "stopwatch.h"
#include "tim.h"
static StopWatch head;
static StopWatch *      ptr;
...
/**
  ...
  */
void Stopwatch_register(StopWatch *stopwatch) {
  ptr = &head;
  //跳转到链表尾
...
/**
  ...
  */
void Stopwatch_tic(StopWatch *stopwatch) {
...
/**
  ...
  */
fp32 Stopwatch_toc(StopWatch *stopwatch) {
  if (stopwatch->enable){
...
/**
  ...
  */
fp32 Stopwatch_disable(StopWatch *stopwatch) {
  if (stopwatch->enable) { 
    int32_t this_us = htim6.Instance->CNT;
    fp32 ret=1e-2 *stopwatch-> dms + 1.0e-6f * (this_us- stopwatch->last_us);
    stopwatch->dms    = 0;
...
void Stopwatch_hook(void) {
    StopWatch *      hookptr = &head;//不使用静态变量保证可重入性
  //遍历每一个链表
