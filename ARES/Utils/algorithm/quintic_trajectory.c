...
/**
  ...
  * @param    ms         持续时间(毫秒)
  */
void quinticTraj_init(QuinticTraj *traj, fp32 s0, fp32 v0, fp32 a0, fp32 st, fp32 vt, fp32 at, uint32_t ms) {
  traj->t = ms;
#define t traj->t
#define t2 (t * t)
#define t3 (t * t * t)
...
/**
  ...
  * @param    traj      五次多项式轨迹结构体
  */
void quinticTraj_start(QuinticTraj *traj) {
...
/**
  ...
  * @param    a         返回当前的加速度
  */
void quinticTraj_calc(QuinticTraj *traj, int8_t dir, fp32 *s, fp32 *v, fp32 *a) {
  fp32 T;
  if (xTaskGetTickCount() - traj->timebase > traj->t) {
    T = traj->t;
  } else {
