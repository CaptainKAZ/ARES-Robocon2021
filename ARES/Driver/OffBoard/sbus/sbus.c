...
#define SBUS_FILTER_COEFF 0.01f
...
void Sbus_init(void) {
  ...
  sbus.filtered.coeff = SBUS_FILTER_COEFF;
...
static void parseSbus(volatile const uint8_t *sbusBuf) {
  sbus.real.channel[0] = (((sbusBuf[1] | (sbusBuf[2] << 8)) & 0x07ff) - SBUS_VALUE_OFFSET) / SBUS_VALUE_MAX;
  sbus.real.channel[1] = ((((sbusBuf[2] >> 3) | (sbusBuf[3] << 5)) & 0x07ff) - SBUS_VALUE_OFFSET) / SBUS_VALUE_MAX;
  sbus.real.channel[2] =
      ((((sbusBuf[3] >> 6) | (sbusBuf[4] << 2) | (sbusBuf[5] << 10)) & 0x07ff) - SBUS_VALUE_OFFSET) / SBUS_VALUE_MAX;
  sbus.real.channel[3] = ((((sbusBuf[5] >> 1) | (sbusBuf[6] << 7)) & 0x07ff) - SBUS_VALUE_OFFSET) / SBUS_VALUE_MAX;
  sbus.real.channel[4] = ((((sbusBuf[6] >> 4) | (sbusBuf[7] << 4)) & 0x07ff) - SBUS_VALUE_OFFSET) / SBUS_VALUE_MAX;
  sbus.real.channel[5] =
      ((((sbusBuf[7] >> 7) | (sbusBuf[8] << 1) | (sbusBuf[9] << 9)) & 0x07ff) - SBUS_VALUE_OFFSET) / SBUS_VALUE_MAX;
  sbus.real.channel[6] = ((((sbusBuf[9] >> 2) | (sbusBuf[10] << 6)) & 0x07ff) - SBUS_VALUE_OFFSET) / SBUS_VALUE_MAX;
  sbus.real.channel[7] = ((((sbusBuf[10] >> 5) | (sbusBuf[11] << 3)) & 0x07ff) - SBUS_VALUE_OFFSET) / SBUS_VALUE_MAX;
  sbus.real.channel[8] = (((sbusBuf[12] | (sbusBuf[13] << 8)) & 0x07ff) - SBUS_VALUE_OFFSET) / SBUS_VALUE_MAX;
  sbus.real.channel[9] = ((((sbusBuf[13] >> 3) | (sbusBuf[14] << 5)) & 0x07ff) - SBUS_VALUE_OFFSET) / SBUS_VALUE_MAX;

  sbus.real.updateTime = xTaskGetTickCount();
}
...
/**
  ...
  */
void Sbus_lpf(void) {
  if (xTaskGetTickCount() - sbus.filtered.updateTIme > 0) {
    fp32 t                   = 0.001f * (xTaskGetTickCount() - sbus.filtered.updateTIme);
    sbus.filtered.updateTIme = xTaskGetTickCount();
    for (uint8_t i = 0; i < 4; i++) {
      sbus.filtered.channel[i] =
          (sbus.filtered.coeff * sbus.filtered.channel[i] + t * sbus.real.channel[i]) / (sbus.filtered.coeff + t);
    }
  }
}
...
void Sbus_guard(uint32_t *errorReg, uint32_t *errorTime) {
  *errorReg  = 0;
  *errorTime = 0;
  if (xTaskGetTickCount() - sbus.real.updateTime > SBUS_MONITOR_TIMEOUT) {
    SET_BIT(*errorReg, MONITOR_ERROR_EXIST);
    SET_BIT(*errorReg, MONITOR_ERROR_LOST);
    Sbus_restrat();
  }
  for (uint8_t i = 0; i < 10; i++) {
    if (__fabs(sbus.real.channel[i]) > 1.0f) {
      SET_BIT(*errorReg, MONITOR_ERROR_EXIST);
