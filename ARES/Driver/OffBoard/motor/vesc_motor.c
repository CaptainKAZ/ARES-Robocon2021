...
void VESC_Motor_RxHook(CAN_Frame *frame) {
  ...
  fp32    last_angle = 0;
  int8_t  index      = 127; //127 means full
  ...
  for (uint8_t i = 0; i < 8; i++) {
    if (vesc_motor[i].general.info.type == VESC_MOTOR) {
      ...
      if ((frame->id >> 8) == CAN_PACKET_STATUS_4) {
        ...
        vesc_motor[index].general.status.angle =
            DEG2RAD(((frame->data[4] << 8 | frame->data[5]) / 50.0f)) - vesc_motor[index].general.status.zero;
      } else {
        return;
      }
      break;
  ...
  case CAN_PACKET_STATUS:
    ...
    vesc_motor[index].general.status.speed =
        (frame->data[0] << 24 | frame->data[1] << 16 | frame->data[2] << 8 | frame->data[3])/7.0f;
  ...
  case CAN_PACKET_STATUS_4:
    ...
    last_angle                                   = vesc_motor[index].general.status.angle;
...
void VESC_Motor_Execute(uint8_t counter) {
  if (VESC_Motor_Mutex == NULL) {
    VESC_Motor_Mutex = xSemaphoreCreateMutexStatic(&VESC_Motor_MutexBuffer);
  }
  xSemaphoreTake(VESC_Motor_Mutex, portMAX_DELAY);
    ...
    if (--vesc_motor[i].general.instruct.timeout > 0) {
      ...
      if (alt_instruct == INSTRUCT_ALTERNATIVE) {
        alt_instruct = vesc_motor[i].general.alt_controller_update(
            (Motor *)&vesc_motor[i], vesc_motor[i].general.alt_controller, vesc_motor[i].general.alt_controller_param);
      }
      //指令未超时
      switch (alt_instruct) {
      case INSTRUCT_CURRENT:
        ...
        break;
      case INSTRUCT_SPEED:
        buffer_append_int32(buffer, (int32_t)(vesc_motor[i].general.instruct.set*7.0f), &send_index);
        VESC_Motor_Command(vesc_motor[i].general.info.device,
      ...
      //指令超时，清空指令
      vesc_motor[i].general.instruct.timeout = 0;
      vesc_motor[i].general.instruct.type    = INSTRUCT_EASE;
      vesc_motor[i].general.instruct.set     = 0;
